...
class TestMapPage:
    ...
    @allure.description(""" Нажатие на кнопку выхода и проверка 
                        что переходим на страницу авторизации""")
    @allure.severity("CRITICAL")
    @pytest.mark.positive
    def test_successful_logout(self, logged_in_map_page):
    ...
    @allure.description(""" Ввод локации в поле поиска и 
                        проверка успешного выполнения поиска""")
    @allure.severity("CRITICAL")
    @pytest.mark.parametrize("location", ["Нижний Новгород"])
    @pytest.mark.positive
    def test_find_location(self, logged_in_map_page, location):
        map_page = logged_in_map_page
    ...
    def test_location_not_found(self, logged_in_map_page, not_exist_location):
        map_page = logged_in_map_page
